...
void svc_handler_c(uint32_t exc_return_val, uint32_t main_stack_ptr_val)
{
	printf("[SVC Handler] LR: 0x%X\r\n", (unsigned int)exc_return_val);
	printf("[SVC Handler] MSP Backup: 0x%X \r\n", (unsigned int)main_stack_ptr_val);
	...
	if (exc_return_val & 0b100) //Test bit 2 of EXC_RETURN
	{
	...
	{
		stack_frame_ptr = (uint32_t *)main_stack_ptr_val; //if 0, stacking used MSP
	...
	if (svc_num == 0xA)
		*stack_frame_ptr = stacked_r0 + stacked_r1; //return r0 + r1
